...
export function SiteHeader({ primaryCtaHref = '/boeken', primaryCtaLabel = 'Boek Nu' }: Props) {
  ...
  return (
    <header
      className="bg-gradient-to-r from-blue-200/95 via-teal-200/90 to-emerald-300/82 backdrop-blur-lg shadow-md border-b border-emerald-300/60 sticky top-0 z-[999] relative overflow-visible pt-[env(safe-area-inset-top)]"
    >
      ...
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 overflow-visible pl-[calc(1rem+env(safe-area-inset-left))] pr-[calc(1rem+env(safe-area-inset-right))] sm:pl-[calc(1.5rem+env(safe-area-inset-left))] sm:pr-[calc(1.5rem+env(safe-area-inset-right))] lg:pl-[calc(2rem+env(safe-area-inset-left))] lg:pr-[calc(2rem+env(safe-area-inset-right))]">
        <div className="flex items-center justify-between h-20 md:h-24 py-4">
          ...
          <div className="flex items-center gap-2 md:gap-4 lg:gap-6 min-w-0">
            ...
            {/* Desktop: Word oppasser (alleen PC, net voor Boek Nu) */}
            <Link
              href="/verzorger-aanmelden"
              className="hidden md:inline-flex items-center justify-center px-3 lg:px-4 h-9 py-2 rounded-full text-sm font-medium text-emerald-800 bg-emerald-50/90 border border-emerald-200/80 hover:bg-emerald-100/90 hover:text-emerald-900 hover:border-emerald-300/80 transition whitespace-nowrap"
            >
              Word oppasser
            </Link>
